...
export class MerchantService {
  ...
  getUserDetail() {
    return lastValueFrom(
      this.requestService.getJSON(`${this.path}/get-detail`, { isLoading: true }),
    );
  }
  ...
  getOrder(data?: any) {
    return lastValueFrom(
      this.requestService.getJSON(`${this.path}/order-info`, { data, isLoading: true }),
    );
  }
  getOrderDetail(id: string) {
    return lastValueFrom(
      this.requestService.getJSON(`${this.path}/order-detail/${id}`, { isLoading: true }),
    );
  }
  ...
  getOrderOverallStat() {
    const path = '/api/orders';
    return lastValueFrom(this.requestService.getJSON(`${path}/overall-stats`, { isLoading: true }));
  }
}
